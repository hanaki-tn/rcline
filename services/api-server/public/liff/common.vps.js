/* ---- BEGIN: visible debug overlay (temp) ---- */
(function(){
  const box = document.createElement('div');
  box.id = 'dbg-overlay';
  box.style.cssText = 'position:fixed;z-index:99999;left:0;top:0;right:0;background:rgba(0,0,0,.85);color:#0f0;font:12px/1.5 monospace;max-height:40%;overflow:auto;padding:8px;white-space:pre-wrap';
  box.textContent = '[BOOT] overlay ready';
  document.addEventListener('DOMContentLoaded', ()=> {
    document.body.prepend(box);
    log('DOM ready: ' + document.readyState);
    log('URL: ' + location.href);
  }, {once:true});
  function log(msg){ box.textContent += '\n' + msg; }
  window._dbglog = log;

  // グローバルエラーを捕捉
  window.addEventListener('error', e => {
    log('ERROR: '+ (e.message || e.error?.message || e.toString()));
  });
  window.addEventListener('unhandledrejection', e => {
    log('UNHANDLED REJECTION: '+ (e.reason?.message || e.reason || 'unknown'));
  });

  // 既存の showDebugLog を可視化にも流す
  const origShow = window.showDebugLog;
  window.showDebugLog = function(msg, level){
    try { log(`[${(level||'info').toUpperCase()}] ${msg}`); } catch(_){}
...
/* ---- END: visible debug overlay (temp) ---- */
